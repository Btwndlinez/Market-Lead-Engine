        ...
        <div className="section">
          <div className={`result-panel ${result.error ? 'result-error' : ''}`} id="result-panel">
            ...
            <div className="result-panel-body">
              {result.error ? (
                <div style={{ padding: '20px', background: 'rgba(239, 68, 68, 0.1)', borderRadius: '12px', border: '1px solid #ef4444', color: '#ef4444' }}>
                  <p style={{ fontWeight: 700 }}>Operation Failed</p>
                  <pre style={{ margin: '8px 0 0', fontSize: '13px', opacity: 0.8 }}>{JSON.stringify(result, null, 2)}</pre>
                </div>
              ) : (
                  ...
                  {(() => {
                    ...
                    if (result.ai_score !== undefined) {
                      ...
                      templateContent.push(
                        <div key="ai-report" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                          <div style={{
                            display: 'flex',
                            alignItems: 'center',
                            ...
                            <div>
                              <p style={{ fontSize: '11px', fontWeight: 700, opacity: 0.6, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '4px' }}>AI Reliability Score</p>
                              <h4 style={{ fontSize: '18px', fontWeight: 800 }}>Intelligent Analysis Complete</h4>
                            </div>
                            <div style={{
                              width: '60px',
                              height: '60px',
                          ...
                          {result.ai_reason && (
                            <div style={{ padding: '20px', background: 'var(--bg-secondary)', borderRadius: '16px', border: '1px solid var(--border)' }}>
                              <p style={{ fontSize: '10px', fontWeight: 700, color: 'var(--fg-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '8px' }}>Rationale</p>
                              <p style={{ fontSize: '15px', fontWeight: 600, color: 'var(--fg)', lineHeight: 1.5 }}>{result.ai_reason}</p>
                            </div>
                          )}
                          ...
                          {result.next_action && (
                            <div style={{
                              ...
                              <div style={{ color: 'var(--accent)', width: 24, height: 24 }}><BoltIcon /></div>
                              <div>
                                <p style={{ fontSize: '10px', fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase' }}>Recommended Action</p>
                                <p style={{ fontSize: '16px', fontWeight: 700 }}>{result.next_action}</p>
                              </div>
                            </div>
                          )}
                        </div>
                    ...
                    // 2. Leakage Template
                    if (result.leakage_detected !== undefined) {
                      templateRendered = true;
                      templateContent.push(
                        <div key="leak-report" style={{
                          padding: '24px',
                          ...
                          <p style={{ fontSize: '48px', fontWeight: 800, color: result.leakage_detected > 0 ? 'var(--accent)' : 'var(--fg)', marginBottom: '4px' }}>{result.leakage_detected}</p>
                          <p style={{ fontSize: '12px', fontWeight: 700, opacity: 0.6, textTransform: 'uppercase' }}>Revenue Leaks Detected</p>
                        </div>
                      );
                    }
                      ...
                      templateContent.push(
                        <div key="generic-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '12px' }}>
                          {Object.entries(result).map(([key, value]) => {
                            if (key === 'success' || key === 'status' || key === 'message') return null;
                            return (
                              <div key={key} style={{ padding: '16px', background: 'var(--bg-secondary)', borderRadius: '12px', border: '1px solid var(--border)' }}>
                                <p style={{ fontSize: '10px', fontWeight: 700, color: 'var(--fg-muted)', textTransform: 'uppercase', marginBottom: '4px' }}>{key.replace(/_/g, ' ')}</p>
                                <p style={{ fontSize: '14px', fontWeight: 600 }}>{typeof value === 'object' ? JSON.stringify(value) : String(value)}</p>
                              </div>
                            );
                          })}
                        </div>
